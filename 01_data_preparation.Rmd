...
sample_metadata <- read_tsv("data/DMB0154_metadata.tsv.gz") %>%
    rename(sample=1)
```
...
read_counts <- read_tsv("data/DMB0154_counts.tsv.gz") %>%
    rename(genome=1)
```
...
genome_coverage <- read_tsv("data/DMB0154_coverage.tsv.gz") %>%
    rename(genome=1)
```
...
genome_metadata <- read_tsv("data/DMB0154_mag_info.tsv.gz") %>%
...
genome_tree <- read_tree("data/DMB0154.tree")
