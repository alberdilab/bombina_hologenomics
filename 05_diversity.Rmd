...
###mixed models
```{r alpha_div_meta, comment"", message=FALSE, warning=FALSE}
# join the data with the metadata on chytrid fungus
alpha_div_meta <- alpha_div %>%
  left_join(sample_metadata, by = join_by(sample == sample))

# Phylogenetic mixed model, alpha diversity explained by Bd_status
Model_phylogenetic_random <- lmer(phylogenetic ~ Bd_status + (1 | Location), 
                                data = alpha_div_meta)
## make this more pretty?
summary(Model_phylogenetic_random)

# functional
Model_functional_random <- lmer(functional ~ Bd_status + (1 | Location), 
                                data = alpha_div_meta)

# look for model parameters and significance
## make this more pretty?
summary(Model_functional_random)
# overall significance of multi-level factors (applies Type III Anove by default)
## make this more pretty?
anova(Model_functional_random)

# reduce model complexity by backward elimination of nonsignificant fixed and random effects
# it looks that the random treatment of location isn't appropriate: discuss this with Antton / Ostaizka 
step(Model_functional_random)
# random effects testing via likelihood ratio tests
ranova(Model_functional_random)

```
# Beta diversity

```{r beta_div, comment="", message=FALSE, warning=FALSE}
# prep to fix the column names
rownames<-colnames(genome_counts_filt)[-1]
colnames<-colnames(genome_counts_filt)[-1]
...
beta_q0n <- genome_counts_filt %>%
  ...
  select_if(~!all(. == 0)) %>%
  hillpair(., q = 0)
# make the column names and rownames re-appear
beta_q0n<-lapply(beta_q0n, 'dimnames<-', list(rownames, colnames))
...
beta_q1n <- genome_counts_filt %>%
  ...
  hillpair(., q = 1)
# make the column names and rownames re-appear
beta_q1n<-lapply(beta_q1n, 'dimnames<-', list(rownames, colnames))

# Shannon diversity, with genome tree?
...
# make the column names and rownames re-appear
beta_q1p<-lapply(beta_q1p, 'dimnames<-', list(rownames, colnames))
...
# make the column names and rownames re-appear
beta_q1f<-lapply(beta_q1f, 'dimnames<-', list(rownames, colnames))
...
## Hill numbers on the bd positive and bd negative groups
```{r hill_numbers, comment="", message=FALSE, warning=FALSE}
# calculate Hill numbers for each sample
hill_profile <- genome_counts_filt %>%
    column_to_rownames(., "genome") %>%
    filter(rowSums(. != 0, na.rm = TRUE) > 0) %>%
    select_if(~!all(. == 0)) %>%
    hilldiv(., q = c(0,1,2))

# transpose and merge with metadata and add Bd status
df_hill<-data.frame(t(hill_profile))
df_hill$Status<-sample_metadata_ordered$Bd_status

# test species richness (q=0)
wilcox.test(q0~Status, data = df_hill)

# test Shannon richness (q=1)
wilcox.test(q1~Status, data = df_hill)

# test Shannon richness (q=2)
wilcox.test(q2~Status, data = df_hill)


## and plot q1 (nearly sign)
ggplot(df_hill, aes(x=Status, y= q1, fill = Status))+
  geom_boxplot(outlier.shape=NA)+
  geom_jitter(width = 0.2, alpha = 0.5)+
  theme_classic()+
  labs(y = "Hill Number (q = 1)", x = "Health status")+
  stat_compare_means(method = "wilcox.test")

## and plot q2 (sign)
ggplot(df_hill, aes(x=Status, y= q2, fill = Status))+
  geom_boxplot(outlier.shape=NA)+
  geom_jitter(width = 0.2, alpha = 0.5)+
  theme_classic()+
  labs(y = "Hill Number (q = 2)", x = "Health status")+
  stat_compare_means(method = "wilcox.test")
